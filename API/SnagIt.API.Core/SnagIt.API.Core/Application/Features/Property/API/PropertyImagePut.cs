        ...
        public class Handler : IRequestHandler<Command, PropertyDto>
        {
            ...
            public async Task<PropertyDto> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    ...
                    var dto = JsonConvert.DeserializeObject<PropertyImageAssignmentPutDto>(requestBody);
                    var command = AssignImageToProperty.Command.Create(dto, request.PropertyId, request.UserId, request.UserName);
