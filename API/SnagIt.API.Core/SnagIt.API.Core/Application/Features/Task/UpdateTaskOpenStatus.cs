...
{
    public class UpdateTaskOpenStatus
    {
        public class Command : IRequest
        {
            private Command(
                TaskStatusPutDto data,
                Guid taskId,
                Guid propertyId, 
                Guid userId, 
                string userName)
            ...
            public TaskStatusPutDto Data { get; }
        ...
        public class Validator : AbstractValidator<Command>
        {
            public Validator()
            {
                RuleFor(query => query.Data)
                    .NotNull()
                    .SetValidator(new TaskStatusPutDtoValidator());
            ...
            private class TaskStatusPutDtoValidator : AbstractValidator<TaskStatusPutDto>
            {
                public TaskStatusPutDtoValidator()
                {
                    RuleFor(data => data.Open)
                        .NotNull();
                }
            }
        }
        ...
        public class Handler : IRequestHandler<Command>
        {
            ...
            public async Task Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                await Create(request, task, property, cancellationToken);
            }
            ...
            {
                if (task.TaskDetail.Open.Equals(request.Data.Open))
                {
                    return;
                }

                task.UpdateStatus((bool)request.Data.Open, property.OwnerId);
